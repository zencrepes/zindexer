...
// Before using jira2md, cleans the text content to replace users with their GitHub credentials
const cleanrJiraContent = (content: string, users: any[], i: any) => {
  ...
  if (i !== undefined && i.attachments !== undefined && i.attachments.totalCount > 0) {
    for (const attachment of i.attachments.edges) {
      ...
      textContent = textContent.replace(regexFiles, `xxyyzzLBxxyyzz${prepString(attachment.node.filename)}xxyyzzRBxxyyzzxxyyzzLPxxyyzz${prepString(attachment.node.remoteBackupUrl)}xxyyzzRPxxyyzz`);

      // Find and replace all image attachments (!file.png|thumbnail!)
      const regex = new RegExp(`!${attachment.node.filename}\\|.*?!`, 'g');
      textContent = textContent.replace(regex, `xxyyzzLBxxyyzz${prepString(attachment.node.filename)}xxyyzzRBxxyyzzxxyyzzLPxxyyzz${prepString(attachment.node.remoteBackupUrl)}xxyyzzRPxxyyzz`);    
    }
  }
